...
export default function App() {
  ...
  const [weeklyMenuRegenPending, setWeeklyMenuRegenPending] = useState(false);
  ...
  useEffect(() => {
    ...
    if (isNewWeek) {
      const gymReset = workoutPlan && profile.gymEnabled;
      if (gymReset) {
      ...
      // Queue menu regeneration after gym reset — handled sequentially in a separate effect
      if (generatedMenu && profile.menuEnabled) setWeeklyMenuRegenPending(true);
      const gymMsg  = gymReset ? ' La rutina se ha reiniciado.' : '';
      const menuMsg = (generatedMenu && profile.menuEnabled) ? ' Regenerando menú...' : '';
      showSuccess(`¡Nueva semana!${gymMsg}${menuMsg}`);
    }
...
<body>
  ...
  // Trigger weekly menu regeneration once any ongoing AI generation is done
  useEffect(() => {
    if (!weeklyMenuRegenPending || isAIGenerating || !isDataLoaded) return;
    if (!generatedMenu || !profile.menuEnabled || !profile.name) {
      setWeeklyMenuRegenPending(false);
      return;
    }
    setWeeklyMenuRegenPending(false);
    menuCooldown.start();
    handleGenerateMenu(profile, goals, profile.weight > 0 ? profile.weight : 70);
  }, [weeklyMenuRegenPending, isAIGenerating]); // eslint-disable-line react-hooks/exhaustive-deps
...
Devuélveme SOLO la nueva tabla en formato Markdown, similar a la anterior pero con ejercicios diferentes o variaciones que mantengan el estímulo. NO incluyas ninguna explicación, solo la tabla Markdown.`;
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${themeStyles.mainBg} pb-24 font-sans selection:${themeStyles.accentMuted}`}>
        ...
        {activeSection === 'hoy' && !!profile.name && (() => {
          ...
          {activeSection === 'gym' && (
            <GymTab
              ...
              habits={habits}
              todayStr={todayStr}
              gymDayDone={gymDayDone}
              gymRoutineDates={gymRoutineDates}
              gymSubTab={gymSubTab}
              setGymSubTab={setGymSubTab}
              planSubTab={planSubTab}
              setPlanSubTab={setPlanSubTab}
              gymDay={gymDay}
              setGymDay={setGymDay}
              gymInfoExpanded={gymInfoExpanded}
              setGymInfoExpanded={setGymInfoExpanded}
              gymTipsExpanded={gymTipsExpanded}
              setGymTipsExpanded={setGymTipsExpanded}
              expandedExSection={expandedExSection}
              setExpandedExSection={setExpandedExSection}
              isGeneratingWorkout={isGeneratingWorkout}
              workoutProgressMsg={workoutProgressMsg}
              workoutNeedsRegeneration={workoutNeedsRegeneration}
              isAIGenerating={isAIGenerating}
              workoutCooldown={workoutCooldown}
              ...
              user={user}
              setHabits={setHabits}
              parsedWorkoutSections={parsedWorkoutSections}
              handleToggleGymDay={handleToggleGymDay}
              handleGymDayDateChange={handleGymDayDateChange}
              handleToggleExercise={handleToggleExercise}
              handleGenerateWorkout={handleGenerateWorkout}
              showError={showError}
              showSuccess={showSuccess}
              showGymSetupBanner={!profile.gymEnabled && !dismissedPrompts.includes('add_gym_goal')}
              onDismissGymSetupBanner={() => dismissPrompt('add_gym_goal')}
              onOpenGymSetup={() => { setProfileWizardStep(3); setProfileModalTab('entrenamiento'); setEditProfile({ ...profile, allergies: Array.isArray(profile.allergies) ? profile.allergies : [], dislikedFoods: profile.dislikedFoods || '' }); setEditWeight(profile.weight > 0 ? profile.weight.toString() : ''); setDismissedSuggestions([]); setIsGoalModalOpen(true); }}
              gymTimeHint={gymTimeHint}
            />
          )}
        </AnimatePresence>
      </main>
      ...
      <AnimatePresence>
        {isGoalModalOpen && (
          <ProfileModal
            ...
            wizardMenuPicked={wizardMenuPicked}
            setWizardMenuPicked={setWizardMenuPicked}
            wizardGymPicked={wizardGymPicked}
            setWizardGymPicked={setWizardGymPicked}
      ...
      {/* ProfileModal-dead-code-placeholder — keeping original JSX in {false&&()} for reference; will be removed on next cleanup */}
      {false && isGoalModalOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
